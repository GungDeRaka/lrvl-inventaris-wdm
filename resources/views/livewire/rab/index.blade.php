<div class="space-y-6">
    ...
    {{-- Header Halaman --}}
    <div
        class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-4 border-gray-200">
        <div>
        ...
        {{-- TOMBOL AJUKAN (Muncul untuk Penjaga Gudang DAN Kepala Gudang) --}}
    ...
    @if ($actionItems->isNotEmpty())
        <div class="bg-white rounded-xl shadow-md overflow-hidden {{ $cardColor }} ring-1 ring-gray-900/5">
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                ...
                <span
                    class="px-3 py-1 rounded-full bg-white border border-gray-200 text-xs font-bold text-gray-600 shadow-sm">
                        ...
                        @foreach ($actionItems as $rab)
                            <tr wire:click="showDetail({{ $rab->id }})"
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    {{ $rab->status == 'menunggu_verifikasi'
                                        ? 'bg-purple-100 text-purple-700 border-purple-200'
                                        : ($rab->status == 'disetujui_bendahara'
                                ...
                                <td class="px-6 py-4 text-right">
                                    <button
                                        class="text-gray-400 hover:text-indigo-600 group-hover:translate-x-1 transition-transform">
                        ...
                        <tr wire:click="showDetail({{ $rab->id }})"
                            ...
                            <td class="px-6 py-4">
                                @php
                                    ...
                                    } elseif (str_contains($rab->status, 'menunggu')) {
                                        $statusColor = 'bg-yellow-100 text-yellow-800 border-yellow-200';
                                    } elseif ($rab->status == 'disetujui_bendahara') {
    ...
    @if ($showDetailModal && $selectedRab)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                {{-- Modal Content --}}
                <div
                    class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-200">
                    ...
                    <div class="bg-indigo-900 px-6 py-4 flex justify-between items-center">
                        <h3 class="text-lg leading-6 font-bold text-white" id="modal-title">
                            Detail Pengajuan RAB <span
                                class="text-indigo-200 font-normal">#{{ $selectedRab->id }}</span>
                        </h3>
                    ...
                    <div class="px-6 py-6 max-h-[70vh] overflow-y-auto bg-gray-50">
                        ...
                        {{-- Info Card --}}
                        <div
                            class="bg-white p-5 rounded-lg border border-gray-200 shadow-sm mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wide">Judul
                                    Pengajuan</label>
                                <p class="text-lg font-bold text-gray-900 mt-1">{{ $selectedRab->judul }}</p>
                            </div>
                            <div>
                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wide">Status Saat
                                    Ini</label>
                                <div class="mt-1">
                                    <span
                            ...
                            <div class="md:col-span-2">
                                <label class="text-xs font-bold text-gray-400 uppercase tracking-wide">Keterangan /
                                    Alasan</label>
                                <div
                                    class="mt-1 p-3 bg-gray-50 rounded border border-gray-200 text-sm text-gray-700 italic">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase">
                                            Sumber Dana</th>
                                        <th class="px-4 py-3 text-center text-xs font-bold text-gray-500 uppercase">Jml
                                        </th>
                                        ...
                                        <tr>
                                            <td class="px-4 py-4 text-sm text-gray-900">
                                                ...
                                                @if (auth()->user()->peran == 'penjaga_gudang' && $selectedRab->status == 'disetujui_bendahara')
                                                    @if ($item->barang_id)
                                                        {{-- TAMPILAN JIKA RESTOCK (Hanya Info) --}}
                                                        <div
                                                            class="mt-3 p-3 bg-green-50 border border-green-200 rounded-md">
                                                            <p
                                                                class="text-xs font-bold text-green-700 flex items-center">
                                                            ...
                                                            <p class="text-xs text-green-600 mt-1">
                                                                Sistem akan menambahkan stok ke barang:
                                                                <strong>{{ \App\Models\Barang::find($item->barang_id)->nama_barang ?? 'Barang' }}</strong>
                                                            </p>
                                                        </div>
                                                    @else
                                            ...
                                            <td class="px-4 py-4 text-sm text-center font-bold">
                                                @if ($editingItemId === $item->id)
                                                    <div class="flex items-center justify-center space-x-1">
                                                        <input type="number" wire:model="editJumlah"
                                                            class="w-16 p-1 text-center text-xs border rounded focus:ring-indigo-500">
                                                        <button wire:click="saveItemBendahara({{ $item->id }})"
                                            ...
                                            @if (auth()->user()->peran == 'bendahara' && $selectedRab->status == 'menunggu_bendahara')
                                                <td class="px-4 py-4 text-center">
                                                    <div class="flex justify-center space-x-2">
                                                        <button wire:click="editItemBendahara({{ $item->id }})"
                                                            class="p-1 text-blue-600 hover:bg-blue-100 rounded border border-transparent hover:border-blue-200 transition">
                                ...
                                <tfoot class="bg-gray-50">
                                    <tr>
                                        <td colspan="4"
                                            class="px-4 py-3 text-right font-bold text-gray-700 uppercase text-xs">
                                            Total Anggaran</td>
                                        <td class="px-4 py-3 text-right font-bold text-indigo-700 text-base">
                                            Rp
                                            {{ number_format($selectedRab->items->sum('harga_total'), 0, ',', '.') }}
                                        </td>
                                        @if (auth()->user()->peran == 'bendahara' && $selectedRab->status == 'menunggu_bendahara')
                                            <td></td>
                                        @endif
                        ...
                        {{-- Form Catatan --}}
                        <div class="mt-6">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Catatan Persetujuan /
                                Penolakan</label>
                            <textarea wire:model="catatan_kepala" rows="3"
                                class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
                                placeholder="Tambahkan catatan untuk pengguna lain..." @if (auth()->user()->peran == 'penjaga_gudang') disabled @endif></textarea>
                        </div>
                    ...
                    <div
                        class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row sm:justify-end gap-3">
                        <button wire:click="closeModal"
                        ...
                        @if (auth()->user()->peran == 'kepala_gudang')
                            @if ($selectedRab->status == 'diajukan')
                                ...
                                <button wire:click="teruskanKeBendahara"
                                    class="w-full sm:w-auto inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none sm:text-sm transition">
                                    Setuju & Teruskan ke Bendahara
                                </button>
                            @elseif($selectedRab->status == 'menunggu_verifikasi')
                                <button wire:click="verifikasiAkhir"
                                    class="w-full sm:w-auto inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none sm:text-sm transition">
                        ...
                        @elseif(auth()->user()->peran == 'penjaga_gudang' && $selectedRab->status == 'disetujui_bendahara')
                            <button wire:click="laporBarangDatang"
                                class="w-full sm:w-auto inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-orange-600 text-base font-medium text-white hover:bg-orange-700 focus:outline-none sm:text-sm transition">
