<div>
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 mt-1">Selamat Datang, <span class="font-semibold text-primary">{{ Auth::user()->name }}</span>!</p>
    </div>

    {{-- Kartu Statistik --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {{-- Card Total Unit --}}
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition duration-200 flex items-center">
    ...
    @if ($permintaanMasuk->isNotEmpty())
        <div class="mb-10 bg-white shadow-lg rounded-xl border border-orange-100 overflow-hidden">
             <div class="px-6 py-4 border-b border-gray-200 bg-orange-50 flex items-center justify-between">
                ...
                <span class="bg-orange-200 text-orange-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{{ $permintaanMasuk->count() }} Baru</span>
            </div>
                    ...
                    <tbody class="text-gray-700 text-sm">
                        @foreach ($permintaanMasuk as $permintaan)
                            <tr class="border-b border-gray-200 hover:bg-orange-50 transition duration-150">
                                ...
                                <td class="py-3 px-6">{{ \Carbon\Carbon::parse($permintaan->waktu_pinjam)->format('d M, H:i') }}</td>
        ...
        <div class="mb-10 bg-white shadow-lg rounded-xl border border-cyan-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-cyan-50 flex items-center justify-between">
                <h2 class="text-lg font-bold text-cyan-700 flex items-center">
                 ...
                 <span class="bg-cyan-200 text-cyan-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{{ $permohonanPerpanjangan->count() }} Pengajuan</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full table-auto">
                    <thead class="bg-gray-50 text-gray-600 uppercase text-xs leading-normal">
                    ...
                    <tbody class="text-gray-700 text-sm">
                        @foreach ($permohonanPerpanjangan as $permohonan)
                            <tr class="border-b border-gray-200 hover:bg-cyan-50 transition duration-150">
                                ...
                                <td class="py-3 px-6">{{ \Carbon\Carbon::parse($permohonan->waktu_kembali)->format('d M, H:i') }}</td>
                                <td class="py-3 px-6 text-center whitespace-nowrap">
                                     <div class="flex item-center justify-center gap-2">
        ...
        <div class="mb-10 bg-white shadow-lg rounded-xl border border-indigo-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-indigo-50 flex items-center justify-between">
                <h2 class="text-lg font-bold text-indigo-700 flex items-center">
                 ...
                 <span class="bg-indigo-200 text-indigo-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{{ $menungguKonfirmasi->count() }} Menunggu</span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full table-auto">
                    <thead class="bg-gray-50 text-gray-600 uppercase text-xs leading-normal">
                                ...
                                <td class="py-3 px-6">
                                    @foreach ($transaksi->barangs as $barang)
                                        <div class="flex items-center justify-between mb-1">
                                            <span>{{ $barang->nama_barang }}</span>
                                            <span class="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded">{{ $barang->pivot->kuantitas }} unit</span>
                                        </div>
                                    @endforeach
                                </td>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 sticky top-0 z-20 bg-gray-100 py-2">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            ...
            <button wire:click="openReportModal"
                ...
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m8-4V3a1 1 0 00-1-1H8a1 1 0 00-1 1v10a1 1 0 001 1h8a1 1 0 001-1zM7 10a1 1 0 011-1h8a1 1 0 011 1v10H7V10z"></path></svg>
                Laporan
            </button>
        </div>
    </div>
    ...
    <div class="bg-white shadow-xl rounded-xl overflow-hidden border border-gray-100 relative">
        ...
        <div class="overflow-x-auto">
            <table class="w-full table-auto">
                <thead class="bg-purple-50 text-purple-900 uppercase text-xs font-bold tracking-wider border-b border-purple-100">
                    <tr>
                ...
                <tbody class="text-gray-700 text-sm divide-y divide-gray-100">
                    @forelse ($transaksis as $transaksi)
                        @php
                            ...
                            $isJatuhTempo = ($transaksi->status == 'dipinjam') && \Carbon\Carbon::parse($transaksi->waktu_kembali)->isPast();
                            
                            $statusClass = match ($transaksi->status) {
                                'dipinjam', 'disetujui' => $isJatuhTempo ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-yellow-100 text-yellow-700 border border-yellow-200',
                            ...
                            $isMendekatiBatas = ($transaksi->status == 'dipinjam' || $transaksi->status == 'disetujui') &&
                                                $waktuKembali->between(now(), now()->addMinutes(30));
                        @endphp
                        
                        <tr class="hover:bg-purple-50/50 transition duration-150 {{ $isJatuhTempo ? 'bg-red-50/30' : '' }}">
                            {{-- Barang --}}
                            <td class="px-6 py-4 align-top">
                                <ul class="space-y-1">
                                    @foreach ($transaksi->barangs as $barang)
                                        <li class="flex items-start">
                                            <span class="font-bold text-gray-800 mr-1">({{ $barang->pivot->kuantitas }})</span>
                                            <div>
                                                <span class="block font-medium text-gray-900">{{ $barang->nama_barang }}</span>
                                                <span class="text-xs text-gray-500 block">{{ $barang->ruangan->nama_ruangan }}</span>
                                            </div>
                                        </li>
                                    @endforeach
                                </ul>
                            ...
                            {{-- Peminjam --}}
                            <td class="px-6 py-4 align-top">
                                <button wire:click="showSiswaDetail({{ $transaksi->siswa->id }})" class="font-semibold text-primary hover:text-purple-800 hover:underline text-left">
                            ...
                            {{-- Lokasi --}}
                            <td class="px-6 py-4 align-top">
                                <span class="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs font-medium">{{ $transaksi->ruang_pemakaian }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 align-top text-xs">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <span class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($transaksi->waktu_pinjam)->format('d M Y, H:i') }}</span>
                                    
                                    <span class="text-gray-500 mt-1">Kembali:</span>
                                    @if ($transaksi->waktu_pengembalian_aktual)
                                        <span class="font-bold text-green-600">{{ \Carbon\Carbon::parse($transaksi->waktu_pengembalian_aktual)->format('d M Y, H:i') }}</span>
                                    @else
                                        <span class="font-medium {{ $isJatuhTempo ? 'text-red-600 animate-pulse' : 'text-gray-900' }}">
                            ...
                            <td class="px-6 py-4 align-top text-center">
                                <span class="px-3 py-1 text-xs font-bold uppercase rounded-full {{ $statusClass }} inline-block shadow-sm">
                                ...
                                <div class="flex flex-col gap-2 items-center">
                                    @if ($isMendekatiBatas || $isJatuhTempo)
                                        @php
                                            ...
                                            $pesan = "PENGINGAT: Peminjaman barang '{$barangList}' ".($isJatuhTempo ? "telah TERLAMBAT" : "akan berakhir segera").". Mohon segera dikembalikan. Terima kasih.";
                                        @endphp
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" x-transition>
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
                <div class="flex justify-between items-center p-6 border-b bg-gray-50 rounded-t-xl flex-shrink-0">
                    ...
                    <button wire:click="closeTransactionModal" class="text-gray-400 hover:text-gray-600 text-3xl leading-none">&times;</button>
                </div>
                ...
                <form wire:submit.prevent="simpanPeminjaman" class="flex flex-col flex-1 min-h-0">
                    <div class="p-6 overflow-y-auto flex-1 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="nis" class="block text-sm font-medium text-gray-700 mb-1">Cari NIS Siswa</label>
                                    <div class="relative">
                                    ...
                                    @error('siswaDitemukan') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                ...
                                @if ($siswaDitemukan)
                                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-start">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-blue-700 mt-1">Kelas: {{ $siswaDitemukan->kelas }} | {{ $siswaDitemukan->no_hp }}</p>
                                        </div>
                                    </div>
                                @elseif(!empty($nis))
                                    <div class="bg-red-50 p-3 rounded-lg border border-red-100 text-red-600 text-sm flex items-center">
                                ...
                                @if (!empty($keranjang))
                                    <div class="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
                                        <div class="px-3 py-2 bg-gray-100 border-b border-gray-200 text-xs font-bold text-gray-500 uppercase">
                                        ...
                                        <div class="max-h-48 overflow-y-auto p-2 space-y-2">
                                            @foreach ($keranjang as $index => $item)
                                                <div class="bg-white p-3 rounded shadow-sm border border-gray-100 flex justify-between items-center">
                                                    <div>
                                                        <span class="text-sm font-bold text-gray-800 block">{{ $item['nama'] }}</span>
                                                        <span class="text-xs text-gray-500 block">Asal: {{ $item['asal'] }}</span>
                                                    </div>
                                                    <div class="flex items-center space-x-1">
                                                        <button type="button" wire:click="decrementKuantitas({{ $index }})" class="w-6 h-6 rounded bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold text-xs">-</button>
                                                        <span class="text-sm w-6 text-center font-semibold">{{ $item['kuantitas'] }}</span>
                                                        <button type="button" wire:click="incrementKuantitas({{ $index }})" class="w-6 h-6 rounded bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold text-xs">+</button>
                                                        <button type="button" wire:click="hapusDariKeranjang({{ $index }})" class="ml-2 text-red-500 hover:text-red-700">
                                ...
                                @endif
                                @error('keranjang') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            {{-- Kolom Kanan --}}
                            <div class="space-y-4">
                                <div class="relative">
                                    <label for="searchBarang" class="block text-sm font-medium text-gray-700 mb-1">Cari & Tambah Barang</label>
                                    ...
                                    @if (!empty($barangDitemukan))
                                        <div class="absolute z-10 w-full mt-1 bg-white rounded-lg shadow-xl border border-gray-200 max-h-60 overflow-y-auto">
                                            <ul class="py-1">
                                                    ...
                                                    @if ($barang->jumlah_saat_ini > 0)
                                                        <li wire:click="tambahKeKeranjang({{ $barang->id }}, '{{ addslashes($barang->nama_barang) }}', '{{ $barang->ruangan->nama_ruangan }}')"
                                                            ...
                                                            <div class="flex justify-between items-center">
                                                                <div>
                                                                    <span class="font-medium text-gray-900 block">{{ $barang->nama_barang }}</span>
                                                                    <span class="text-xs text-gray-500 block">{{ $barang->ruangan->nama_ruangan }}</span>
                                                                </div>
                                                                <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-bold">{{ $barang->jumlah_saat_ini }} Stok</span>
                                                            </div>
                                                        </li>
                                                    @else
                                                        <li class="px-4 py-3 text-gray-400 cursor-not-allowed bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                                                             <span>{{ $barang->nama_barang }}</span>
                                                             <span class="text-xs font-bold text-red-400">Habis</span>
                                                        </li>
                                                    @endif
                                                @empty
                                                    <li class="px-4 py-3 text-gray-500 text-sm text-center">Barang tidak ditemukan.</li>
                                                @endforelse
                                            </ul>
                                        </div>
                                    @endif
                                ...
                                <div>
                                    <label for="ruang_pemakaian" class="block text-sm font-medium text-gray-700 mb-1">Ruang Pemakaian</label>
                                    <select wire:model="ruang_pemakaian" id="ruang_pemakaian" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-purple-500 focus:border-purple-500">
                                        ...
                                        @foreach($ruangans as $ruangan)
                                            ...
                                            >
                                                ...
                                                @if($isAsalRuangan)
                                                    (Asal: {{ $this->asalRuangan[$ruangan->nama_ruangan]->pluck('nama_barang')->join(', ') }})
                                                @endif
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('ruang_pemakaian') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                </div>

                                <div>
                                    <label for="waktu_kembali" class="block text-sm font-medium text-gray-700 mb-1">Waktu Pengembalian</label>
                                    <input type="datetime-local" id="waktu_kembali" wire:model="waktu_kembali"
                                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-purple-500 focus:border-purple-500">
                                    @error('waktu_kembali') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 border-t bg-gray-50 rounded-b-xl flex justify-end space-x-3 flex-shrink-0">
                        <button type="button" wire:click="closeTransactionModal" class="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg shadow-sm hover:bg-gray-50 font-medium transition">Batal</button>
                        <button type="submit" wire:loading.attr="disabled" class="px-5 py-2.5 bg-purple-700 text-white rounded-lg shadow-md hover:bg-purple-800 font-bold transition flex items-center">
                            <svg wire:loading wire:target="simpanPeminjaman" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                            Simpan Peminjaman
        ...
        {{-- ... (Kode Modal Pengembalian dengan styling serupa di atas) ... --}}
         <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
                ...
                <p class="text-sm text-gray-600 mb-4">Peminjam: <strong class="text-gray-900">{{ $transaksiTerpilih->siswa->nama }}</strong></p>
                <div class="space-y-3 mb-6 max-h-60 overflow-y-auto pr-1">
                    ...
                    <div class="flex items-center justify-between bg-gray-50 p-3 rounded border border-gray-100">
                        ...
                        <div class="flex items-center">
                             ...
                             <input type="number" wire:model="kerusakanItems.{{ $barang->id }}" class="w-16 text-sm border-gray-300 rounded focus:ring-purple-500 focus:border-purple-500 p-1" min="0" max="{{ $barang->pivot->kuantitas }}">
                        </div>
                    </div>
                    @endforeach
                </div>
                <div class="flex justify-end space-x-2">
                    <button wire:click="$set('transaksiIdUntukDikembalikan', null)" class="px-4 py-2 bg-gray-100 text-gray-700 rounded hover:bg-gray-200 font-medium text-sm">Batal</button>
                    <button wire:click="prosesPengembalian" class="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700 font-bold text-sm shadow-sm">Konfirmasi</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if ($siswaDetail)
         <div wire:click="closeModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 cursor-pointer">
            <div @click.stop class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 cursor-default relative">
                <button wire:click="closeModal" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>
                <h3 class="text-lg font-bold text-gray-900 mb-4 border-b pb-2">Detail Peminjam</h3>
                <div class="space-y-3">
                    <div class="grid grid-cols-3 text-sm"><span class="text-gray-500">Nama</span><span class="col-span-2 font-medium text-gray-900">{{ $siswaDetail->nama }}</span></div>
                    <div class="grid grid-cols-3 text-sm"><span class="text-gray-500">NIS</span><span class="col-span-2 font-medium text-gray-900">{{ $siswaDetail->nis }}</span></div>
                    <div class="grid grid-cols-3 text-sm"><span class="text-gray-500">Kelas</span><span class="col-span-2 font-medium text-gray-900">{{ $siswaDetail->kelas }}</span></div>
                    <div class="grid grid-cols-3 text-sm"><span class="text-gray-500">No. HP</span><span class="col-span-2 font-medium text-gray-900">{{ $siswaDetail->no_hp }}</span></div>
                    <div class="grid grid-cols-3 text-sm"><span class="text-gray-500">Status</span>
                        <span class="col-span-2">
                            @if($siswaDetail->is_ditangguhkan) <span class="text-red-600 font-bold bg-red-50 px-2 py-0.5 rounded-full text-xs">Ditangguhkan</span> @else <span class="text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-full text-xs">Aktif</span> @endif
                        </span>
                    </div>
                </div>
            </div>
